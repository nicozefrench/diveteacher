...
from app.core.logging_config import log_stage_start, log_stage_complete
...
async def get_graphiti_client() -> Graphiti:
    ...
    if _graphiti_client is None:
        ...
        logger.info("   LLM: Gemini 2.5 Flash-Lite (Google Direct)")
        logger.info("   Embeddings: text-embedding-3-small (OpenAI - 1536 dims)")
        logger.info("   Cost: ~$1-2/year (99.7% cheaper than Haiku!)")
        logger.info("   Rate Limits: 4K RPM (Tier 1) - No throttling!")
        ...
        logger.info("🔒 Production-Ready Mode: Sequential (Simple)")
        logger.info("   • No bulk mode (not needed with Gemini)")
        logger.info("   • No SafeQueue (4K RPM = plenty)")
        logger.info("   • Expected: 100% success rate, ultra-low cost")
        ...
        logger.info("   • LLM: Gemini 2.5 Flash-Lite (GeminiClient)")
        logger.info("   • Embeddings: OpenAI text-embedding-3-small (1536 dims)")
        logger.info("   • Cross-Encoder: gpt-4o-mini (reranking)")
        logger.info("   • Architecture: ARIA v1.14.0 (Sequential Simple)")
        logger.info("   • Processing: Sequential (no bulk, no SafeQueue)")
        logger.info("   • Cost: ~$1-2/year (99.7% cheaper than Haiku!)")
...
async def ingest_chunks_to_graph(
    ...
    if upload_id:
        ...
        logger.info("🔒 ARIA Pattern: Sequential processing (simple mode)")
        logger.info("   • No bulk mode (not needed)")
        logger.info("   • No SafeQueue (4K RPM = plenty)")
        logger.info("   • Expected: 100% success rate, ultra-low cost")
    else:
        ...
        logger.info("   Mode: Sequential (simple)")
    ...
    logger.info("")
    logger.info("✅ Sequential ingestion complete:")
    logger.info(f"   Total time: {total_time:.2f}s")
    logger.info(f"   Chunks: {successful}/{len(chunks)} ({success_rate:.1f}%)")
    logger.info(f"   Avg time/chunk: {avg_time_per_chunk:.2f}s")
    logger.info("   Mode: Sequential (simple, no SafeQueue)")
    logger.info("   LLM: Gemini 2.5 Flash-Lite (~$1-2/year)")
